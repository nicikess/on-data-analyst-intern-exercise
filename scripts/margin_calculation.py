def add_margin_columns(data):
    ...
    temp_data["target_margin"] = (
        temp_data["Target Retail Price $"] - temp_data["Target FOB $"]
    ) * temp_data["Target Volume"]
    temp_data["actual_margin"] = (
        temp_data["Target Retail Price $"] - temp_data["Actual FOB $"]
    ) * temp_data["Target Volume"]

    temp_data["target_margin_per_piece"] = (
        temp_data["Target Retail Price $"] - temp_data["Target FOB $"]
    )
    temp_data["actual_margin_per_piece"] = (
        temp_data["Target Retail Price $"] - temp_data["Actual FOB $"]
    )
