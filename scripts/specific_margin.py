...
def lowest_and_highest_margin_calculation(margin_data):
    ...
    temp_data_sorted = temp_data.sort_values(
        by=["actual_margin_per_piece"], ascending=False
    )
    # Select top 1 and bottom 1 actual margin
...
def seasonal_margin_calculation(data):
    ...
    for season_data in season_temp_data:
        ...
        (
            lowest_and_highest_margin,
            lowest_and_highest_margin_per_piece,
        ) = lowest_and_highest_margin_calculation(temp_data)
        season_margin.append(
            [lowest_and_highest_margin, lowest_and_highest_margin_per_piece]
        )
...
def specific_margin_calculation(data, margin_data):
    ...
    (
        highest_and_lowest_margin,
        highest_and_lowest_margin_per_piece,
    ) = lowest_and_highest_margin_calculation(margin_data)
