...
def margin_calculation_statistics(data):
    ...
    temp_data_season = temp_data.loc[:,
                       ['Season', 'Target Volume', 'Target FOB $', 'Target Retail Price $', 'Actual FOB $',
    ...
    temp_data_color_update = temp_data.loc[:,
                             ['Color Update', 'Target Volume', 'Target FOB $', 'Target Retail Price $', 'Actual FOB $',
                              'target_margin', 'actual_margin', 'target_margin_per_piece', 'actual_margin_per_piece']]
    margin_data_color_update = temp_data_color_update.groupby('Color Update').mean()

    # Margin vertical
    temp_data_vertical = temp_data.loc[:,
                         ['Vertical', 'Target Volume', 'Target FOB $', 'Target Retail Price $', 'Actual FOB $',
                          'target_margin', 'actual_margin', 'target_margin_per_piece', 'actual_margin_per_piece']]
    margin_data_vertical = temp_data_vertical.groupby('Vertical').mean()

    print(margin_data_vertical.to_string())

    return [margin_data_season, margin_data_gender, margin_data_factory, margin_data_style_update,
            margin_data_color_update, margin_data_vertical]
...
def cost_calculation_statistics(data):
    ...
    # Margin vertical
    temp_data_season = temp_data.loc[:,
                       ['Season', 'UPPER Cost', 'Other Cost', 'BOTTOM Cost', 'LABOR Cost', 'OVERHEAD Cost',
                        'Tooling Cost',
                        'Actual FOB $']]
    ...
    # Margin vertical
    temp_data_gender = temp_data.loc[:,
                       ['Gender', 'UPPER Cost', 'Other Cost', 'BOTTOM Cost', 'LABOR Cost', 'OVERHEAD Cost', 'Tooling Cost',
                             'Actual FOB $']]
    cost_data_gender = temp_data_gender.groupby('Gender').mean()

    # Margin per factory
    temp_data_factory = temp_data.loc[:,
                        ['Factory', 'UPPER Cost', 'Other Cost', 'BOTTOM Cost', 'LABOR Cost', 'OVERHEAD Cost', 'Tooling Cost',
                             'Actual FOB $']]
    cost_data_factory = temp_data_factory.groupby('Factory').mean()

    # Margin per style update
    temp_data_style_update = temp_data.loc[:,
                             ['Style Update', 'UPPER Cost', 'Other Cost', 'BOTTOM Cost', 'LABOR Cost', 'OVERHEAD Cost', 'Tooling Cost',
                             'Actual FOB $']]
    cost_data_style_update = temp_data_style_update.groupby('Style Update', dropna=False).mean()

    # Margin per color update
    temp_data_color_update = temp_data.loc[:,
                             ['Color Update', 'UPPER Cost', 'Other Cost', 'BOTTOM Cost', 'LABOR Cost', 'OVERHEAD Cost', 'Tooling Cost',
                             'Actual FOB $']]
    cost_data_color_update = temp_data_color_update.groupby('Color Update').mean()

    # Margin vertical
    temp_data_vertical = temp_data.loc[:,
                         ['Vertical', 'UPPER Cost', 'Other Cost', 'BOTTOM Cost', 'LABOR Cost', 'OVERHEAD Cost', 'Tooling Cost',
                             'Actual FOB $']]
    cost_data_vertical = temp_data_vertical.groupby('Vertical').mean()

    print(cost_data_style_update.to_string())

    return [cost_data_season, cost_data_gender, cost_data_factory, cost_data_style_update,
            cost_data_color_update, cost_data_vertical]
