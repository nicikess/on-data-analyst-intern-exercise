...
def data_for_visualisation(data):
    ...
    temp_data = temp_data.loc[
        :, ["Style Name", "Vertical", "Target Retail Price $", "Actual FOB $"]
    ]
    ...
    for key, value in temp_data.items():
        temp_data[key] = value.groupby(
            ["Style Name", "Vertical"], as_index=False
        ).mean()
...
def tool_target_margin(data):
    ...
    with col1:
        ...
        st.write(
            "This tool is designed to help you analyze the impact of "
            "Target FOBs (Free on Board) and Retail Prices on the margins of On's products. "
            "The tool is divided into three tabs: Performance Running, Performance Outdoor and Performance All Day. "
            "Each tab contains a histogram plot, that shows the distribution of the margins of the different products "
            "in the respective category. The sliders allow you to increase the Target FOB and Retail Price by a certain percentage. "
            "In the second section a specific On product can be selected and the absolute Price and FOB can be adjusted."
        )
    with col2:
        # Add a logo
        logo_image = "scripts/img/on-running-logo-vector.png"  # Replace with the path to your logo image
        st.image(logo_image, width=130)  # Adjust the width as per your requirements
    ...
    with col1:
        ...
        # Using st.markdown() with markdown syntax for the subsubheading
        st.markdown("<h6>Adjust relative values</h4>", unsafe_allow_html=True)

        # Create two columns
        col1_slider, col2_slider = st.columns([1, 1], gap="large")

        with col1_slider:
            retail_price = st.slider(
                "Increase Retail Price (0% - 100%)", 0, 100, 0, step=10
            )
        with col2_slider:
            actual_cost = st.slider(
                "Increase Target FOB (0% - 100%)", 0, 100, 0, step=10
            )

        retail_fraction = (retail_price / 100) + 1
        actual_fraction = (actual_cost / 100) + 1

        selected_product_data_histo["Target Retail Price $"] = (
            selected_product_data_histo["Target Retail Price $"] * retail_fraction
        )
        selected_product_data_histo["Actual FOB $"] = (
            selected_product_data_histo["Actual FOB $"] * actual_fraction
        )
    ...
    with col2:
        ...
        st.markdown("<h6>Adjust absolute values</h4>", unsafe_allow_html=True)

        selected_product = st.selectbox(
            "Select a Product", selected_product_data["Style Name"].unique()
        )

        # Retrieve the current margin for the selected product
        selected_product_data_filtered = selected_product_data[
            selected_product_data["Style Name"] == selected_product
        ]

        st.write(
            "Current Retail Price:",
            selected_product_data_filtered["Target Retail Price $"].round(0).item(),
        )
        st.write(
            "Current FOB:",
            selected_product_data_filtered["Actual FOB $"].round(0).item(),
        )

        # Subracting two columns results in series, therefore item() method must be applied to get the float value
        margin = (
            (
                selected_product_data_filtered["Target Retail Price $"]
                - selected_product_data_filtered["Actual FOB $"]
            )
            .round(0)
            .item()
        )
        ...
        new_retail_price = st.slider(
            "New Retail Price",
            0,
            500,
            int(selected_product_data_filtered["Target Retail Price $"].item()),
            step=10,
        )
        new_actual_cost = st.slider(
            "New Actual Cost",
            0,
            500,
            int(selected_product_data_filtered["Actual FOB $"].item()),
            step=10,
        )
